...
const excluiAgendamento = async (req, res) => {
  const { id } = req.params;
  try {
    const agendamento = await knex("agendamento").select("*").where({ id }).first();

    if (!agendamento) {
      return res.status(400).json({ mensagem: "Este Cliente ainda não foi cadastrado" });
    }
...
const editarAgendamento = async (req, res) => {
  ...
  try {
    ...
    if (hora_inicio || servicos || data_marcacao) {
      ...
      const resposta = await verificarHorarioBloqueado(
        data_marcacao || agendamentoAtual.data_marcacao,
        horaInicio,
        hora_termino
      );
      if (resposta) {
...
const editarConfirmacaoAgendamento = async (req, res) => {
  ...
  try {
    ...
    const agendamento = await knex("agendamento")
      .update({ confirmacao_agendamento })
...
const listarAgendamentos = async (req, res) => {
  try {
    const agendamentos = await knex("agendamento")
      ...
      .orderBy("agendamento.data_marcacao", "desc")
...
const listarHorariosOcupados = async (req, res) => {
  ...
  try {
    ...
    const bloqueios = await knex("bloqueiohorarios").where("data_bloqueio", "=", data);
    ...
    const horarios = await knex("agendamento").select("hora_inicio").where("data_marcacao", data);
